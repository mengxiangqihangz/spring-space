...
<%-- 	                        	<c:if test="${user.sex eq '0'}">女</c:if> --%>
				...
				<page:PageTag key="${key}"pageNo="${page.pageNo }" pageSize="${page.pageSize }" pageUrl="${pageContext.request.contextPath}/sys/user/list" pageLast="${page.totalPage }"/>
            </div>
            <!--右侧内容结束-->
...
<script>
    ...
    function delUser (Id) {
		if (confirm("你确定要删除么?")) {
			$.post("${pageContext.request.contextPath}/sys/user/delete",{id:Id},
	    			   function(data){
	    			     	if (data=='1'){
	    			    		 alert('删除成功');
	    			    		 document.location.href = "${pageContext.request.contextPath }/sys/user/list"
...
// 	 });
	...
	function fineda(){
		 document.location.href = "${pageContext.request.contextPath }/sys/user/quda?key="+document.getElementById('key').value;
	}
