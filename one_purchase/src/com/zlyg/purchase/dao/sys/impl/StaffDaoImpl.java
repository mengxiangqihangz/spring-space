...
public class StaffDaoImpl implements IStaffDao{
	...
	public PageNavigation<SysStaff> quPageData(PageNavigation<SysStaff> page, SysStaff staff) {
		...
		System.out.println(jdbcTemplate.queryForInt(sqll));
	...
	public int StaffEdit(SysStaff staff) throws Exception {	
		String sql = "update Sys_staff set staff_id=?,staff_name=?,staff_sex=?,staff_status=?,"
				+ "staff_source=?,staff_reservation=?,Staff_buildDate=?,Staff_buildMan=?,"
				+ "Staff_visitDate=?,Staff_birthday=?,staff_graduateSchool=?,"
				+ "Staff_education=?,Staff_contact=?,staff_urgencyName=?,Staff_urgencyPhone=?,"
				+ "Staff_jobinterview=?,Staff_Visible=?,user_id=? where id=?";
...
public SysStaff fine(SysStaff staff){
	...
	public String blackList(String id) {
		String sql="update sys_staff set staff_visible='0' where Id=?";
	...
	@Override
	public int getId() {
		String sql="select count(*) from sys_staff";
