...
<%-- 	                        	<c:if test="${staff.sex eq '0'}">女</c:if> --%>
								<c:choose>
									...
									<c:when test="${staff.sex == '-1'}">人妖</c:when>
	                        ...
	                        <td>
	                        	<c:choose>
									<c:when test="${staff.education == '0'}">博士</c:when>
									<c:when test="${staff.education == '1'}">硕士</c:when>
									<c:when test="${staff.education == '2'}">本科</c:when>
									<c:when test="${staff.education == '3'}">专科</c:when>
									<c:when test="${staff.education == '4'}">高中</c:when>
...
<%-- 	                            <a href="#" class="btn btn-primary btn-sm" onclick="editUser (${staff.id});">编辑</a> --%>
	                            <a href="#" class="btn btn-primary btn-sm" onclick="delUser(${staff.id });">洗白</a>
	                        </td>
	                    </tr>
                    </c:forEach>
                    </tbody>
...
<%--                 <% String url = request.getScheme()+"://"+ request.getServerName()+request.getRequestURI()+"?"+request.getQueryString(); %> --%>
				<page:PageTag key="${key}" pageNo="${page.pageNo }" pageSize="${page.pageSize }" pageUrl="${pageContext.request.contextPath}/sys/staff/blackList" pageLast="${page.totalPage }"/>
            </div>
            <!--右侧内容结束-->
...
<!-- 							         <input type="text" class="form-control" id="status" name="status" placeholder="请输入员工状态"> -->
							   ...
							   <div class="form-group">
							      ...
							      <div class="col-sm-8">
							         <input type="radio" checked="checked" name="visible" value="1"/>是 &nbsp;&nbsp;
...
<script>
    		...
    		$.post("${pageContext.request.contextPath}/sys/staff/edit"
    			,$("#addForm").serialize(),
    			   function(data){
    			     if (data=='1'){ 
    			    	 alert('编辑成功');
    			    	 document.location.href = "${pageContext.request.contextPath }/staff/list"
    			     }else{
    			    	 alert("编辑失败");
    			     }
    		});
    ...
    function delUser (Id) {
		if (confirm("你确定要洗白 么?")) {
			$.post("${pageContext.request.contextPath}/sys/staff/washWhite",{id:Id},
	    			   function(data){
	    			     	if (data=='1'){
// 	    			    		 alert('洗白成功');
	    			    		 document.location.href = "${pageContext.request.contextPath }/sys/staff/blackList"
	    			     	}else{
	    			    		 alert("洗白失败");
	    			     	}
	    		});
...
// 	 });
	...
	function fineda(){
		 document.location.href = "${pageContext.request.contextPath }/sys/staff/quhda?key="+document.getElementById('key').value;
	}
